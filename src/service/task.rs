...
impl crate::server::MyService {
    pub async fn _create_task(&self, request: Request<Task>) -> Result<Response<Task>, Status> {
        ...
        self._colink_internal_storage_update(&user_id, &format!("tasks:{}", task_id), &payload)
            .await?;
        for i in 1..task.participants.len() {
            let core_addr = self
                ._colink_internal_storage_read(
                    &user_id,
    ...
    pub async fn _confirm_task(
        ...
        let mut payload = vec![];
        task.encode(&mut payload).unwrap();
        self._colink_internal_storage_update(
            &user_id,
    ...
    pub async fn _inter_core_sync_task(
        ...
        let user_id = Self::get_user_id(request.metadata());
        if !self
            ._colink_internal_storage_contains(
                &user_id,
                &format!("tasks:{}", request.get_ref().task_id),
            )
            .await?
    ...
    async fn remove_task_old_status(&self, user_id: &str, task: &Task) -> Result<(), Status> {
        ...
        let list = self
            ._colink_internal_storage_read(user_id, &list_key)
            .await?;
    ...
    async fn add_task_new_status(&self, user_id: &str, task: &Task) -> Result<(), Status> {
        ...
        .unwrap();
        let key_path = self
            ._colink_internal_storage_update(user_id, &latest_key, &payload)
            .await?;
        let mut list = if self
            ._colink_internal_storage_contains(user_id, &list_key)
            .await?
        {
            let list = self
                ._colink_internal_storage_read(user_id, &list_key)
    ...
    async fn generate_decision(
        ...
        let user_consent_bytes = self
            ._colink_internal_storage_read(user_id, "user_consent")
            .await?;
